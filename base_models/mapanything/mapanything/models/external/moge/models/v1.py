# References:
#   https://github.com/facebookresearch/map-anything/blob/main/mapanything/models/external/moge/models/v1.py
...
class MoGeModel(nn.Module):
    ...
    def init_weights(self):
        ...
        # Path to gradio/LoopModels/dinov2
        dinov2_path = os.path.abspath(os.path.join(current_dir, "../../../../../../../LoopModels/dinov2"))
        
        if os.path.exists(dinov2_path):
            print(f"Loading dinov2 (for MoGe) from local path: {dinov2_path}")
            # Note: local load might need verify=False or similar if checking signatures, but typically not for local
            model = torch.hub.load(dinov2_path, self.encoder, source='local', pretrained=True)
