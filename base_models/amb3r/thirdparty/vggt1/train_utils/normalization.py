...
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/vggt/train_utils/normalization.py
...
def normalize_camera_extrinsics_and_points_batch(
    ...
    if scale_by_points:
        ...
        # dist_median = dist.median(dim=[1, 2, 3]).values
        dist_median = dist.flatten(1).median(dim=1).values



        if pred_points is not None:
            pred_dist = pred_points.norm(dim=-1)
            pred_dist_median = pred_dist.flatten(1).median(dim=1).values
            # pred_dist_sum = (pred_dist * point_masks).sum(dim=[1,2,3])
            # pred_valid_count = point_masks.sum(dim=[1,2,3])
            # avg_scale_pred = (pred_dist_sum / (pred_valid_count + 1e-3)).clamp(min=1e-6, max=1e6)

            # scale_diff = (pred_dist_median).clamp(min=1e-6, max=1e6)
            # avg_scale = avg_scale / scale_diff
            avg_scale = dist_median / pred_dist_median
        ...
        return new_extrinsics[:, :, :3], cam_points, new_world_points, depths, avg_scale
