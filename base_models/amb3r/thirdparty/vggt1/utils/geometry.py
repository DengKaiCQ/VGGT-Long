...
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/vggt/utils/geometry.py

import os
import torch
import numpy as np
from typing import Union
from vggt1.dependency.distortion import apply_distortion, iterative_undistortion, single_undistortion



# Note: This function relies on the `closed_form_inverse_se3` function 
# which is already present in the provided `geometry.py` file.

def unproject_depth_map_to_point_map_torch(
    depth_map: Union[np.ndarray, torch.Tensor],
    extrinsics_cam: Union[np.ndarray, torch.Tensor],
    intrinsics_cam: Union[np.ndarray, torch.Tensor],
) -> Union[np.ndarray, torch.Tensor]:
    """
    Unproject a batch of depth maps to 3D world coordinates in a vectorized manner.

    This function supports both NumPy arrays and PyTorch tensors as input. If any input
    is a PyTorch tensor, all inputs are converted to tensors and the computation
    is performed on the same device as the input `depth_map`.

    Args:
        depth_map (Union[np.ndarray, torch.Tensor]): Batch of depth maps of shape
            (S, H, W, 1) or (S, H, W).
        extrinsics_cam (Union[np.ndarray, torch.Tensor]): Batch of camera extrinsic
            matrices of shape (S, 3, 4).
        intrinsics_cam (Union[np.ndarray, torch.Tensor]): Batch of camera intrinsic
            matrices of shape (S, 3, 3).

    Returns:
        Union[np.ndarray, torch.Tensor]: Batch of 3D world coordinates of shape (S, H, W, 3),
            matching the input type.
    """
    # 1. Determine the backend (torch or numpy) and ensure consistent types
    is_torch = any(isinstance(arg, torch.Tensor) for arg in [depth_map, extrinsics_cam, intrinsics_cam])

    if is_torch:
        lib = torch
        einsum_fn = torch.einsum
        # Use the device of the depth_map if it's a tensor, otherwise default to CPU
        device = depth_map.device if isinstance(depth_map, torch.Tensor) else "cpu"

        # Convert all inputs to torch tensors on the correct device
        depth_map = torch.as_tensor(depth_map, dtype=torch.float32, device=device)
        extrinsics_cam = torch.as_tensor(extrinsics_cam, dtype=torch.float32, device=device)
        intrinsics_cam = torch.as_tensor(intrinsics_cam, dtype=torch.float32, device=device)
    else:
        lib = np
        einsum_fn = np.einsum
        # Ensure all inputs are numpy arrays
        if not isinstance(depth_map, np.ndarray): depth_map = np.array(depth_map, dtype=np.float32)
        if not isinstance(extrinsics_cam, np.ndarray): extrinsics_cam = np.array(extrinsics_cam, dtype=np.float32)
        if not isinstance(intrinsics_cam, np.ndarray): intrinsics_cam = np.array(intrinsics_cam, dtype=np.float32)

    # 2. Reshape depth map from (S, H, W, 1) to (S, H, W) if necessary
    if depth_map.ndim == 4 and depth_map.shape[-1] == 1:
        depth_map = depth_map.squeeze(-1)

    S, H, W = depth_map.shape

    # 3. Extract camera intrinsic parameters
    fu = intrinsics_cam[..., 0, 0]
    fv = intrinsics_cam[..., 1, 1]
    cu = intrinsics_cam[..., 0, 2]
    cv = intrinsics_cam[..., 1, 2]

    # 4. Create a grid of pixel coordinates (u, v)
    if is_torch:
        u, v = lib.meshgrid(
            lib.arange(W, device=device, dtype=torch.float32),
            lib.arange(H, device=device, dtype=torch.float32),
            indexing="xy",
        )
    else:
        u, v = lib.meshgrid(
            lib.arange(W, dtype=np.float32),
            lib.arange(H, dtype=np.float32),
            indexing="xy",
        )

    # 5. Unproject pixels to camera coordinates
    # Reshape intrinsics for broadcasting: (S,) -> (S, 1, 1)
    fu = fu.reshape(S, 1, 1)
    fv = fv.reshape(S, 1, 1)
    cu = cu.reshape(S, 1, 1)
    cv = cv.reshape(S, 1, 1)
    ...
    # Stack to form camera coordinates points: (S, H, W, 3)
    cam_coords = lib.stack([x_cam, y_cam, z_cam], axis=-1)

    # 6. Transform camera coordinates to world coordinates
    ...
    # Reshape translation for broadcasting: (S, 3) -> (S, 1, 1, 3)
    t_cam_to_world = t_cam_to_world.reshape(S, 1, 1, 3)

    # Apply rotation (R.T) and translation
    # The operation is equivalent to (cam_coords @ R.T) + t for each item in the batch
    if is_torch:
        R_t = R_cam_to_world.transpose(-2, -1)
    else:
        R_t = np.transpose(R_cam_to_world, (0, 2, 1))

    world_points = einsum_fn("shwi,sij->shwj", cam_coords, R_t) + t_cam_to_world
