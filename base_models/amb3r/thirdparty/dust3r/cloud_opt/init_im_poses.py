...
# --------------------------------------------------------
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/dust3r/cloud_opt/init_im_poses.py
...
from dust3r.cloud_opt.commons import edge_str, i_j_ij, compute_edge_scores, compute_edge_scores2
...
@torch.no_grad()
def init_minimum_spanning_tree(self, return_tree=False, **kw):
    """ Init all camera poses (image-wise and pairwise poses) given
        an initial set of pairwise estimations.
    """
    if return_tree:
        return minimum_spanning_tree(self.imshapes, self.edges, self.pred_i, self.pred_j, self.conf_i, self.conf_j, self.im_conf, self.min_conf_thr,
                                     self.device, has_im_poses=self.has_im_poses, verbose=self.verbose, return_tree=True)
    device = self.device
...
def compute_distance_matrix(imshapes, edges, conf_i, conf_j):
    n_imgs = len(imshapes)
    sparse_graph = -dict_to_sparse_graph(compute_edge_scores(map(i_j_ij, edges), conf_i, conf_j))
    dist_matrix = np.full((n_imgs, n_imgs), np.inf)

    for (i, j), score in sparse_graph.items():
...
def minimum_spanning_tree(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, min_conf_thr,
                          device, has_im_poses=True, niter_PnP=10, verbose=True, return_tree=False):
    n_imgs = len(imshapes)
    sparse_graph = -dict_to_sparse_graph(compute_edge_scores(map(i_j_ij, edges), conf_i, conf_j))
