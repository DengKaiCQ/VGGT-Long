...
# --------------------------------------------------------
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/dust3r/cloud_opt/commons.py
...
def edge_conf2(conf_i, conf_j, edge):
    return float(conf_j[edge].mean())


def compute_edge_scores(edges, conf_i, conf_j):
    return {(i, j): edge_conf(conf_i, conf_j, e) for e, (i, j) in edges}

def compute_edge_scores2(edges, conf_i, conf_j):
    return {(i, j): edge_conf2(conf_i, conf_j, e) for e, (i, j) in edges}
