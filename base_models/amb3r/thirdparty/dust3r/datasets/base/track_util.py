...
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/dust3r/datasets/base/track_util.py
...
def build_tracks_by_depth(extrinsics, intrinsics, world_points, depths, point_masks, images, pos_rel_thres=0.05, neg_epipolar_thres=16, 
    ...
    """
    # assert the shape of inputs
    assert extrinsics.shape[0] == intrinsics.shape[0] == world_points.shape[0] == depths.shape[0] == point_masks.shape[0], \
        f"Shape mismatch: {extrinsics.shape[0]}, {intrinsics.shape[0]}, {world_points.shape[0]}, {depths.shape[0]}, {point_masks.shape[0]}"
    
    assert extrinsics.shape[1:] == (3, 4), f"Extrinsics shape should be (N, 3, 4), got {extrinsics.shape}"
    assert intrinsics.shape[1:] == (3, 3), f"Intrinsics shape should be (N, 3, 3), got {intrinsics.shape}"
    assert world_points.shape[1:] == (depths.shape[1], depths.shape[2], 3), \
        f"World points shape should match depths, got {world_points.shape} and {depths.shape}"
    assert point_masks.shape[1:] == (depths.shape[1], depths.shape[2]), \
        f"Point masks shape should match depths, got {point_masks.shape} and {depths.shape}"
