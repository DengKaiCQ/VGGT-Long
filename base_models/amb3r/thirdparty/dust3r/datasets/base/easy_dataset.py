...
# --------------------------------------------------------
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/dust3r/datasets/base/easy_dataset.py
...
class EasyDataset:
    ...
    def make_sampler(self, batch_size, shuffle=True, world_size=1, rank=0, drop_last=True):
        ...
        num_of_aspect_ratios = len(self._features)
...
class CatDataset (EasyDataset):
    ...
    @property
    def _features(self):
        features = self.datasets[0]._features
        for dataset in self.datasets[1:]:
            assert tuple(dataset._features) == tuple(features)
        return features
