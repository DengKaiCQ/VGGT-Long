# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/moge/moge/train/losses.py
...
def scale_invariant_alignment(
    ...
    scale = align_points_scale(pred_points_lr.flatten(-3, -2), gt_points_lr.flatten(-3, -2), lr_mask.flatten(-2, -1) / gt_points_lr[..., 2].flatten(-2, -1).clamp_min(1e-2), trunc=trunc)


    valid = scale > 0
    scale = torch.where(valid, scale, 0.0)

    if detach:
        scale = scale.detach()

    # Apply scale only (no shift)
    pred_points = scale[..., None, None, None] * pred_points

    return pred_points, scale

def scale_z_shift_alignment(
    pred_points: torch.Tensor, 
    gt_points: torch.Tensor, 
    mask: torch.Tensor, 
    align_resolution: int = 64, 
    ...
    (pred_points_lr, gt_points_lr), lr_mask = mask_aware_nearest_resize((pred_points, gt_points), mask=mask, size=(align_resolution, align_resolution))
    scale, shift = align_points_scale_xyz_shift(pred_points_lr.flatten(-3, -2), gt_points_lr.flatten(-3, -2), lr_mask.flatten(-2, -1) / gt_points_lr[..., 2].flatten(-2, -1).clamp_min(1e-2), trunc=trunc)
    valid = scale > 0
