...
# --------------------------------------------------------
# References:
#   https://github.com/HengyiWang/amb3r/blob/main/thirdparty/croco/utils/misc.py
...
def init_distributed_mode(args):
    ...
    # e.g. Consider a distributed training setup with 2 nodes, each having 4 GPUs (total WORLD_SIZE of 8):
    # Node 1:
    # Process (GPU) 0: RANK 0, LOCAL_RANK 0
    # Process (GPU) 1: RANK 1, LOCAL_RANK 1
    # Process (GPU) 2: RANK 2, LOCAL_RANK 2
    # Process (GPU) 3: RANK 3, LOCAL_RANK 3
    # Node 2:
    # Process (GPU) 0: RANK 4, LOCAL_RANK 0
...
class NativeScalerWithGradNormCount:
    ...
    def __init__(self, enabled=True):
        # The 'enabled' flag is key. Set it to False for bfloat16.
        self._scaler = torch.cuda.amp.GradScaler(enabled=enabled)

    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        # If the scaler is disabled (i.e., for bfloat16), perform a standard backward pass.
        if not self._scaler.is_enabled():
            loss.backward(create_graph=create_graph)
            if update_grad:
                if clip_grad is not None:
                    assert parameters is not None
                    # No unscaling is needed before clipping
        ...
        # Original logic for FP16 (when scaler is enabled)
...
def save_model(args, epoch, model_without_ddp, optimizer, loss_scaler, fname=None, best_so_far=None, best_relative_so_far=None):
    ...
    if best_relative_so_far is not None: to_save['best_relative_so_far'] = best_relative_so_far
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    ...
    if args.resume is not None:
        ...
        if 'best_relative_so_far' in checkpoint:
            best_relative_so_far = checkpoint['best_relative_so_far']
            print(" & best_relative_so_far={:g}".format(best_relative_so_far))
        print("With optim & sched! start_epoch={:d}".format(args.start_epoch), end='')
    return best_so_far, best_relative_so_far
