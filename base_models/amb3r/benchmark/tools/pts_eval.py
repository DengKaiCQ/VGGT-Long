# References:
#   https://github.com/HengyiWang/amb3r/blob/main/benchmark/tools/pts_eval.py
...
def compute_abs_rel(pts, pts_gt):
    abs_rel = np.nan_to_num(np.linalg.norm(pts - pts_gt, axis=1) / np.linalg.norm(pts_gt, axis=1), nan=-1.0)
    abs_rel_m = abs_rel[abs_rel >= 0.0].mean() * 100
    return abs_rel_m
